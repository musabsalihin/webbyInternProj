...
<x-app-layout>
        ...
        <div class="flex justify-center max-w-7xl mx-auto mt-20 px-4 sm:px-6 lg:px-8">
            <form class="px-12 w-1/2 bg-white rounded-xl" method="post" action="{{route('users.update', ['user' => $user])}}">
                @csrf
                @method('put')
                <div class="block my-6">
                    <label class="block mb-1" for="name">Name</label>
                    <input class="w-full rounded-xl border-gray-400 focus:outline-none focus:border-blue-500" id="name" name="name" type="text" value="{{$user->name}}">
                </div>
                <div class="block my-6">
                    <label class="block mb-1" for="email">Email</label>
                    <input class="w-full rounded-xl border-gray-400 focus:outline-none focus:border-blue-500" id="email" name="email" type="text" value="{{$user->email}}">
                </div>
                <div class="block my-6">
